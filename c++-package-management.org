...
Compiler flags are about as standardized as they are going to be. And that specifying compiler flags beyond where to find dependent packages is a 'toolchain' decision. (* weird expensive optimization flags are an open issue)
