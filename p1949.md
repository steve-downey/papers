...
Detection of un-normalized text is fairly straignt-forward, and GCC 10 already produces a warning. Normalizing to NFC is not much more difficult. In SG16 discussions it has been pointed out that IFNDR is usually reserved for situations where it is difficult or impossible to identify the problem, where for non-normalized text it is merely a metter of code. However, we need feedback from implementers, particularly since this is a potentially breaking change.

In current implementations, compilers already emit incompatible mangling for non-ASCII characters even while they claim ABI compatibility. The situations that would require an ABI break are already an ABI incompatibility in current implementations. The proposed solution would provide most resources required for an ABI specification to ensure compatibility.
...
<Start> := XID_Start, plus 005F	_	LOW LINE
...
~~~
0009          ; # Cc       CHARACTER TABULATION
000A          ; # Cc       LINE FEED (LF)
000B          ; # Cc       LINE TABULATION
000C          ; # Cc       FORM FEED (FF)
000D          ; # Cc       CARRIAGE RETURN (CR)
0020          ; # Zs       SPACE
~~~
...
> An identifier is an arbitrarily long sequence of letters and digits. Each universal-character-name in an identifier[, excluding the first element,]{.add} shall designate a character whose encoding in ISO/IEC 10646 falls into [the therein defined class XID\_CONTINUE.]{.add}[one of the ranges specified in Table 2.]{.rm} The initial element[, if a universal-character-name, shall be a character whose encoding in ISO/IEC 10646 falls into the therein defined class XID_START.]{.add} [shall not be a universal-character-name designating a character value whose encoding falls into one of the ranges specified in Table 3.]{.rm} Upper- and lower-case letters are different. All characters are significant.

In [lex.name], remove table [tab:lex_name_allowed] and [tab:lex_name_disallowed].

In [lex.name], add a new section:

>::: add
> An identifier shall conform to the NFC normalization specified in ISO/IEC 10646. If an identifier is encountered that does not, the program is ill-formed.
>:::
...
>
> **Change**: identifiers that were valid before, containing characters not present in ISO/IEC 10646 XID\_START or XID\_CONTINUE, or in non-NFC normalization format, are now rejected.
>
> **Rationale**: Many confusable identifiers were previously technically allowed but not commonly used. C++23 requires these changes to conform to ISO/IEC 10646 recommendations and to prevent confusion between normalization formats causing compile errors.
>
> **Effect on original feature**: Identifiers are now validated according to ISO/IEC 10646 recommended methods. Identifiers that contain invisible characters are not allowed. Identifiers can only be written in a single way.
>:::
...
::: add
>
> # Annex X: Design choices in application of UAX31
>
> [X.1]{.pnum} Default Identifiers
>
>
...
>
>
> [X.1.1]{.pnum} R1a Restricted Format Characters
>
> Format characters are not additionally restricted beyond what UAX31 provides.
>
> [X.1.2]{.pnum} R1b. Stable Identifiers
>
> No explicit stability guarantees beyond what is provided by UAX31.
>
> [X.2]{.pnum} R2. Immutable Identifiers
>
> Identifiers are not immutable. Additional identifiers may be available in the future.
>
> [X.3]{.pnum} R3. Pattern\_White\_Space and Pattern\_Syntax Characters
>
> C++ defines a profile of characters used for white space and for syntax other than identifiers White space character are
...
>
> [X.4]{.pnum} R4. Equivalent Normalized Identifiers
>
> Identifiers that compare the same under NFC are equivalent.
>
> [X.5]{.pnum} R5. Equivalent Case-Insensitive Identifiers
>
> There are no case insensitive comparisons or case folding
>
> [X.6]{.pnum} R6. Filtered Normalized Identifiers
>
> No filtering is performed
>
> [X.7]{.pnum} R7. Filtered Case-Insensitive Identifiers
> No filtering is performed
>
> [X.8]{.pnum} R8. Hashtag Identifiers
>
> There are no hashtag identifiers
:::
