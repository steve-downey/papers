...
Detection of un-normalized text is fairly straight-forward, and GCC 10 already produces a warning. Normalizing to NFC is not much more difficult. In SG16 discussions it has been pointed out that IFNDR is usually reserved for situations where it is difficult or impossible to identify the problem, where for non-normalized text it is merely a matter of code. However, we need feedback from implementer, particularly since this is a potentially breaking change.
